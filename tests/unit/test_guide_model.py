...
class GuideModelTests(unittest.TestCase):
    def test_private_sources_are_excluded(self):
        manifest = {
            ...
            "sources": [
                {"source_id": "public", "public_label": "Project README", "path_or_origin": "README.md", "authority": "repo", "privacy": "public", "used_for": ["public_guide"]},
                {"source_id": "internal", "path_or_origin": "notes.md", "authority": "repo", "privacy": "public", "used_for": ["model"]},
                {"source_id": "secret", "path_or_origin": ".env", "authority": "repo", "privacy": "private", "used_for": ["none"]},
            ],
        }
        ...
        self.assertIn("[withheld]", all_text)
        self.assertNotIn("Demo", all_text)

    def test_public_disclosure_allows_selected_project_fields(self):
        guides = build_guides({
            "project": {"name": "Demo", "intent": "Test", "product": "Guide output"},
            "disclosure": {"public": {"name": True, "intent": True, "product": False}},
        }, {"schema": "repokernel.source-manifest.v1", "sources": []})
        all_text = "\n".join(guides.values())
        self.assertIn("Demo", all_text)
        self.assertIn("Test", all_text)
        self.assertNotIn("Guide output", all_text)
